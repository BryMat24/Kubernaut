...
class DiagnosisSummarizer:
    ...
    async def classify(self, query: str, messages: list[BaseMessage]) -> DiagnosisResult:
        finalize_prompt = f"""
            ...
            findings), root_cause (root cause if one was found, otherwise null), and
            diagnosis_success (true if the investigation completed and reached an
            evidence-backed conclusion; false if it was inconclusive or you are not
